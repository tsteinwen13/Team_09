...
abstract class NewsgroupSwipeHelper(
    context: Context?, private val recyclerView: RecyclerView,
    internal var buttonWidth:Int)
    : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {

    private var buttonList:MutableList<EditNewsgroupButton>?=null
    lateinit var gestureDetector: GestureDetector
    var swipePosition = -1
    var swipeTreshhold = 0.5f
    var buttonBuffer:MutableMap<Int, MutableList<EditNewsgroupButton>>
    lateinit var removerQueue: LinkedList<Int>
    abstract fun instantiateEditNewsgroupButton(viewHolder: RecyclerView.ViewHolder,
    ...
    override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
        ...
        if(buttonBuffer.containsKey(swipePosition))
            ...
            buttonList!!.clear()
        buttonBuffer.clear()
        swipeTreshhold = 0.5f*buttonList!!.size.toFloat()*buttonWidth.toFloat()
        recoverSwipeItem()
    }
    ...
    override fun onChildDraw(
        ...
        if(actionState == ItemTouchHelper.ACTION_STATE_SWIPE)
        {
            if(dX < 0)
            {
                var buffer : MutableList<EditNewsgroupButton> = ArrayList()
                if(!buttonBuffer.containsKey(pos))
                {
                    instantiateEditNewsgroupButton(viewHolder,buffer)
    ...
    private fun drawButton(c: Canvas, itemView: View, buffer: MutableList<EditNewsgroupButton>, pos: Int, translationX: Float) {
